...
class Session {
  ...
  applySettings({ maxStreak, wrongGuessPolicy, bonusGuessingEnabled, powers } = {}) {
    ...
    if (powers && typeof powers === 'object') {
      for (const key of ['extraSong', 'saveWinnings', 'yearMargin', 'randomCard']) {
  ...
  placeCard(playerId, slotIndex, guesses = {}) {
    if (this.phase !== 'playing' || !this.turn || this.turn.revealed) return null;
    ...
    const titleCorrect = this.bonusGuessingEnabled ? matchTitle(guesses.titleGuess, this.turn.card.title) : null;
    const artistCorrect = this.bonusGuessingEnabled ? matchArtist(guesses.artistGuess, this.turn.card.artist) : null;
    ...
    this.turn.titleCorrect = titleCorrect;
    this.turn.artistCorrect = artistCorrect;
    if (titleCorrect) team.titleHits = (team.titleHits || 0) + 1;
    if (artistCorrect) team.artistHits = (team.artistHits || 0) + 1;
  ...
  snapshot() {
    ...
    if (this.turn) {
      turn = {
        ...
        slotIndex: this.turn.slotIndex,
        correct: this.turn.correct,
