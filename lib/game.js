...
class Session {
  constructor(code, songs) {
    ...
    // 'table' — the host browser is the Spotify device (Web Playback SDK).
    //           Needs a Premium login and speakers on that machine, but no
    //           player ever sees the track, so nothing can be spoiled.
    // 'dj'     — one phone is handed a deep link per song and plays it in its
    //           own Spotify app, through whatever that phone is connected to.
    //           Needs no Spotify credentials at all, so anyone can host.
    this.audioMode = 'dj';
    this.djPlayerId = null;   // which player's device is holding the DJ role
    this.djIsHost = false;    // …or the hosting device itself, when one phone is the whole setup
    // iOS reaches the Spotify app reliably via the custom scheme; the https
    // link only works once that phone has already opened Spotify from a link
    // at least once, and silently falls back to the web player otherwise.
    this.linkStyle = 'scheme'; // 'scheme' = spotify:track:… | 'https' = open.spotify.com/…
    this.powers = {
  ...
  joinTeam(playerId, teamId) {
    ...
    p.teamId = teamId;
    // switching teams? don't leave the old one orphaned + empty behind
    if (oldTeam && oldTeam !== teamId && !this.membersOf(oldTeam).length) {
  ...
  // ---- host team management (lobby only) ------------------------------------
  // Reassign a player to another team, or to no team (teamId null/''), from the
  // table's console. Auto-removes a team left empty by the move.
  adminMovePlayer(playerId, teamId) {
    ...
    const oldTeam = p.teamId;
    if (oldTeam === target) return false; // no-op
    p.teamId = target;
    if (oldTeam && !this.membersOf(oldTeam).length) {
      this.teams = this.teams.filter((t) => t.id !== oldTeam);
    }
  ...
  // Remove a team from the lobby; any members drop back to the unassigned pool.
  adminRemoveTeam(teamId) {
    if (this.phase !== 'lobby') return false;
    const team = this.teams.find((t) => t.id === teamId);
    if (!team) return false;
    for (const p of this.membersOf(teamId)) p.teamId = null;
  ...
  snapshot() {
    ...
    return {
      ...
      djPlayerId: this.djPlayerId,
      djIsHost: this.djIsHost,
